...
$bookedQuery = "SELECT AppointmentDate, AppointmentTime FROM appointment WHERE Status = 'Active'";
...
$appointmentCode = AutoID("appointment", "AppointmentCode", "Ap_", 6);
...
if (isset($_POST['btnsubmit'])) {
    ...
    $checkAvailability = "SELECT * FROM appointment 
    ...
    if (mysqli_num_rows($availabilityResult) > 0) {
        ...
        (AppointmentCode, PatientID, AppointmentDate, AppointmentTime, Name, Email, PhoneNumber, Address, Status)
        VALUES
        ('$appointmentCode', '$pid', '$appointment_date', '$appointment_time', '$name', '$email', '$phone', '$address','$status')";
...
<header class="relative py-24 bg-gradient-to-br from-[#BA9EA8] to-[#7A4E5E] text-white overflow-hidden">
    ...
    <div class="container mx-auto px-6 text-center relative z-10">
        <h1 class="text-5xl md:text-6xl font-bold mb-6">Book an Appointment</h1>
...
<body class="bg-[#FFF8F5] text-[#4C2A36] font-sans">
    <div class="max-w-3xl mx-auto px-6 py-10 bg-white rounded shadow mt-10 mb-10">
        <form id="appointmentForm" action="appointment.php" method="POST" class="space-y-8">
            <input type="hidden" name="AppointmentCode" value="<?php echo $appointmentCode; ?>">
            ...
            <!-- Step 1 -->
            <div id="step1">
                <h2 class="text-xl font-semibold mb-4">Step 1 of 2: Choose Date & Time</h2>
                ...
                <div class="flex justify-between mt-6">
                    <button type="button" onclick="nextStep(1)" class="bg-[#916D7A] text-white px-6 py-2 rounded">Next</button>
                </div>
            </div>

            <!-- Step 2 -->
            <div id="step2" class="hidden">
                <h2 class="text-xl font-semibold mb-4">Step 2 of 2: Your Information</h2>
                <div class="space-y-4">
